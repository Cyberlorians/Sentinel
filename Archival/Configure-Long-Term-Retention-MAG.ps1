<#       
    ...
    .NOTES
        AUTHOR: Original Author Sreedhar Ande for Commercial. Tweaked by Cyberlorians for MAG Tenant.
        LASTEDIT: 6/10/22
...
function Set-TableConfiguration {
    ...
    foreach($QTable in $QualifiedTables) {	
		$TablesApi = "https://management.usgovcloudapi.net/subscriptions/$SubscriptionId/resourcegroups/$LogAnalyticsResourceGroup/providers/Microsoft.OperationalInsights/workspaces/$LogAnalyticsWorkspaceName/tables/$($QTable.TableName)" + "?api-version=2021-12-01-preview"								
...
function Get-AllTables {		
	$AllTables = @()	
    $TablesApi = "https://management.usgovcloudapi.net/subscriptions/$SubscriptionId/resourcegroups/$LogAnalyticsResourceGroup/providers/Microsoft.OperationalInsights/workspaces/$LogAnalyticsWorkspaceName/tables" + "?api-version=2021-12-01-preview"								
...
function Update-TablesRetention {
    ...
    foreach($tbl in $TablesForRetention) {
		$TablesApi = "https://management.usgovcloudapi.net/subscriptions/$SubscriptionId/resourcegroups/$LogAnalyticsResourceGroup/providers/Microsoft.OperationalInsights/workspaces/$LogAnalyticsWorkspaceName/tables/$($tbl.TableName)" + "?api-version=2021-12-01-preview"		
...
Try {
    ...
    Connect-AzAccount -Tenant $TenantID -ContextName 'MyAzContext' -Force -ErrorAction Stop -Environment AzureUSGovernment -UseDeviceAuthentication
